...
export const getExpiryDate = (duration: string): Date => {
  ...
  const safeUnit: keyof typeof ms =
    unit in ms ? (unit as keyof typeof ms) : "d";
  const multiplier = ms[safeUnit] ?? ms.d;

  return new Date(Date.now() + normalizedValue * (multiplier ? multiplier : 1));
};
